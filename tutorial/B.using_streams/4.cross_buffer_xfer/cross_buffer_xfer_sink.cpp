...
// A sink-side version of a task that computes the content of an array
//  based on some other arrays. It also returns a value at the end of the
//  computation. The return value is the 20th parameter and its size is the
//  21st one in hstreams.
HSTREAMS_EXPORT
void compute_sink_1(
    uint64_t N, uint64_t AA, uint64_t BB, uint64_t CC, uint64_t DD,
