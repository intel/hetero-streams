    ...
    for (iter = 0; iter < niter; ++iter) {
        ...
        for (k = 0; k < num_tiles; ++k) {
            ...
            if (k > 0) {
                ...
                hStreams_app_xfer_memory((int)(q_getrf % max_log_str),
            ...
            for (m = k + 1; m < num_tiles; ++m) {
                if (k == 0) {
                    ...
                    hStreams_app_xfer_memory((int)(q_trsm % max_log_str),
            ...
            for (m = k + 1; m < num_tiles; ++m) {
                for (n = k + 1; n < num_tiles; ++n) {
                    if (k == 0) {
                        ...
                        hStreams_app_xfer_memory((int)(q_gemm % max_log_str),
                    ...
                    res = hStreams_app_dgemm((int)(q_gemm % max_log_str),
                                             blasLay, CblasNoTrans,
